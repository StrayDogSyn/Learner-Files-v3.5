      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-xl p-6 border border-white/20">
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto">
              {getCriticalTasks().map((task: Task) => (
                <div key={task.id} className="p-3 bg-red-500/10 rounded-lg border border-red-500/20">
                  <div className="flex items-center justify-between mb-2">
                  ...
                  {task.assignedAgent && (
                    <p className="text-blue-400 text-xs mt-1">Agent: {task.assignedAgent}</p>
                  )}
                </div>
              ))}
              ...
              {getCriticalTasks().length === 0 && (
                <div className="text-center py-8">
                  ...
                  <p className="text-green-400 font-medium">All Critical Tasks Completed!</p>
                </div>
              )}
            </div>
          </div>
        ...
        {systemStatus && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {systemStatus.domains.map((domain: Domain) => (
              <div key={domain.id} className="bg-white/10 backdrop-blur-md rounded-xl p-6 border border-white/20">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-bold text-white">{domain.name}</h3>
                  <div className={`w-3 h-3 rounded-full ${
                    domain.status === 'healthy' ? 'bg-green-500' :
                    domain.status === 'warning' ? 'bg-yellow-500' :
                    'bg-red-500'
                  }`}></div>
                </div>
                
                <p className="text-slate-300 text-sm mb-4">{domain.description}</p>
                
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-400">Tasks:</span>
                    <span className="text-white">
                      {systemStatus.tasks.filter((t: Task) => t.domain === domain.id).length}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-400">Agents:</span>
                    <span className="text-white">
                      {systemStatus.agents.filter((a: Agent) => a.domains.includes(domain.id)).length}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-400">Status:</span>
                    <span className={`capitalize ${
                      domain.status === 'healthy' ? 'text-green-400' :
                      domain.status === 'warning' ? 'text-yellow-400' :
                      'text-red-400'
                    }`}>
                      {domain.status}
                    </span>
