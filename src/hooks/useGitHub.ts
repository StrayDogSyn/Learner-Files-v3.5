...
const convertToProjectCard = (repo: GitHubRepository): ProjectCardData => {
  return {
    id: repo.id.toString(),
...
const loadCachedData = (): Partial<UseGitHubState> | null => {
  try {
...
const saveCachedData = (data: Partial<UseGitHubState>) => {
  try {
    const cacheData = {
      ...data,
      timestamp: Date.now()
...
export const useGitHub = (): UseGitHubReturn => {
  ...
  const searchRepositories = useCallback(async (query: string): Promise<GitHubRepository[]> => {
    try {
