...
const AICaseStudyGenerator: React.FC = () => {
  ...
  const generateCaseStudy = async () => {
    if (!selectedProject && !customPrompt) return;
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="text-center mb-8">
        <div className="flex items-center justify-center gap-3 mb-4">
          <div className="p-3 bg-gradient-to-r from-hunter-green-500 to-hunter-green-600 rounded-full">
            <FileText className="w-8 h-8 text-white" />
          </div>
          <h1 className="text-4xl font-bold bg-gradient-to-r from-hunter-green-400 to-hunter-green-600 bg-clip-text text-transparent">
            AI Case Study Generator
          </h1>
        </div>
        <p className="text-gray-400 text-lg max-w-2xl mx-auto">
          Generate compelling case studies showcasing justice reform impact using AI analysis of project data
        </p>
      </div>
      ...
      <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 mb-8">
        <h2 className="text-xl font-semibold text-white mb-4 flex items-center gap-2">
          <Sparkles className="w-5 h-5 text-hunter-green-400" />
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div>
            ...
            <select
              ...
              title="Choose a project to generate case study"
              className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-hunter-green-500"
            >
              <option value="">Choose a project...</option>
              {projectData.map((project) => (
                <option key={project.name} value={project.name} className="bg-gray-800">
                  {project.name}
                </option>
              ))}
            </select>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-hunter-green-500 resize-none"
              rows={3}
            />
          </div>
        </div>
        ...
        <button
          ...
          disabled={isGenerating || (!selectedProject && !customPrompt)}
          className="mt-4 bg-gradient-to-r from-hunter-green-500 to-hunter-green-600 text-white px-6 py-3 rounded-lg font-medium hover:from-hunter-green-600 hover:to-hunter-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 flex items-center gap-2"
        >
          {isGenerating ? (
            <>
      ...
      <div className="space-y-6">
        {caseStudies.map((caseStudy) => (
          <div key={caseStudy.id} className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h3 className="text-2xl font-bold text-white mb-2">{caseStudy.title}</h3>
                <p className="text-hunter-green-400 font-medium">{caseStudy.client}</p>
              </div>
              <button
                onClick={() => downloadCaseStudy(caseStudy)}
                className="bg-white/10 hover:bg-white/20 border border-white/20 text-white px-4 py-2 rounded-lg transition-all duration-300 flex items-center gap-2"
              >
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-6">
              <div>
                <h4 className="text-lg font-semibold text-white mb-2 flex items-center gap-2">
                  <Target className="w-5 h-5 text-hunter-green-400" />
                  Challenge
                </h4>
            ...
            <div className="mb-6">
              <h4 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                <TrendingUp className="w-5 h-5 text-hunter-green-400" />
                Impact Metrics
              </h4>
              <div className="grid md:grid-cols-3 gap-4">
                {caseStudy.impact.map((metric, index) => (
                  <div key={index} className="bg-white/5 border border-white/10 rounded-lg p-4">
                    <div className="text-2xl font-bold text-hunter-green-400 mb-1">{metric.value}</div>
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-6">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {caseStudy.technologies.map((tech) => (
                    <span key={tech} className="bg-hunter-green-500/20 text-hunter-green-400 px-3 py-1 rounded-full text-sm">
                      {tech}
                    </span>
                  ))}
                </div>
              ...
              <div>
                <h4 className="text-lg font-semibold text-white mb-2 flex items-center gap-2">
                  <Calendar className="w-5 h-5 text-hunter-green-400" />
                  Timeline
                </h4>
            ...
            {/* Testimonial */}
            {caseStudy.testimonial && (
              <div className="bg-hunter-green-500/10 border border-hunter-green-500/20 rounded-lg p-4">
                <h4 className="text-lg font-semibold text-white mb-2 flex items-center gap-2">
            ...
            <div className="text-xs text-gray-500 mt-4">
              Generated on {caseStudy.generatedAt.toLocaleDateString()} at {caseStudy.generatedAt.toLocaleTimeString()}
            </div>
          </div>
        ))}
      </div>
      ...
      {caseStudies.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold text-gray-400 mb-2">No Case Studies Generated Yet</h3>
          <p className="text-gray-500">Select a project and generate your first AI-powered case study</p>
        </div>
      )}
    </div>
  );
