...
export const GlassContainer = forwardRef(({ variant = 'base', blur = 'medium', glow = false, children, className, ...props }, ref) => {
    const blurLevels = {
        light: '8px',
        medium: '16px',
        heavy: '24px'
    ...
    return (_jsx(motion.div, { ref: ref, style: dynamicStyles, className: cn('glass-container', className), initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.6, ease: 'easeOut' }, ...props, children: children }));
});
GlassContainer.displayName = 'GlassContainer';
export const GlassCard = forwardRef(({ hoverable = true, clickable = false, loading = false, children, className, ...props }, ref) => {
    ...
    return (_jsx(GlassContainer, { ref: ref, className: cn('p-6 transition-all duration-300 cursor-default', {
            'cursor-pointer': clickable,
            'hover:shadow-lg': hoverable,
            'animate-pulse': loading
        }, className), ...hoverAnimation, ...clickAnimation, ...props, children: loading ? (_jsx("div", { className: "flex items-center justify-center h-24", children: _jsx("div", { className: "animate-spin rounded-full h-8 w-8 border-2 border-white border-t-transparent" }) })) : (children) }));
});
GlassCard.displayName = 'GlassCard';
export const GlassButton = forwardRef(({ variant = 'primary', size = 'md', glow = false, loading = false, children, className, ...props }, ref) => {
    const sizeClasses = {
    ...
    return (_jsx(motion.button, { ref: ref, style: buttonStyle, className: cn('font-medium rounded-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed', 'hover:scale-105 active:scale-95', 'focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-transparent', {
        ...
        }, sizeClasses[size], className), whileHover: { scale: 1.05 }, whileTap: { scale: 0.95 }, transition: { duration: 0.2 }, disabled: loading, ...props, children: loading ? (_jsxs("div", { className: "flex items-center justify-center", children: [_jsx("div", { className: "animate-spin rounded-full h-4 w-4 border-2 border-current border-t-transparent mr-2" }), "Loading..."] })) : (children) }));
});
GlassButton.displayName = 'GlassButton';
//# sourceMappingURL=glassmorphic-system.js.map