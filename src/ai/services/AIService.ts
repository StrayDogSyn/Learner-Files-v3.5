...
import type { ClaudeRequest } from '../../shared/types/ai';
import {
  DomainType,
  ContentType,
  UserRole,
...
export class AIService {
  ...
  private buildLeadQualificationPrompt(request: LeadQualificationRequest): string {
...
Format the response with clear code blocks and explanations.`;
  ...
  private buildROIAnalysisPrompt(request: ROIAnalysisRequest): string {
...
Format as JSON with these fields: roiPercentage, paybackPeriod, npv, riskLevel, recommendations`;
  ...
  private buildImpactMetricsPrompt(request: ImpactMetricsRequest): string {
...
Format as JSON with these fields: impactScore, affectedPopulation, successProbability, kpis, measurementStrategy`;
  ...
  private parseLeadQualification(response: string): LeadQualificationResponse {
    try {
  ...
  private parseCodeGeneration(response: string, request: CodeGenerationRequest): CodeGenerationResponse {
  ...
  private parseROIAnalysis(response: string): ROIAnalysisResponse {
    try {
      const jsonMatch = response.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
        const parsed = JSON.parse(jsonMatch[0]);
  ...
  private parseImpactMetrics(response: string): ImpactMetricsResponse {
    try {
      const jsonMatch = response.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
        const parsed = JSON.parse(jsonMatch[0]);
