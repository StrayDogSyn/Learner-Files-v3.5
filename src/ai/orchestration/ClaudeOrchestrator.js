...
import { errorHandler } from '@repo/database';
export class ClaudeOrchestrator {
    constructor(apiKey) {
        ...
        Object.defineProperty(this, "contextManager", {
            enumerable: true,
            configurable: true,
            writable: true,
     ...
     */
    async generateContent(request, userTier = 'free') {
        const startTime = Date.now();
        const requestId = this.generateRequestId();
        try {
            // Check rate limits
            const rateLimitCheck = await this.rateLimiter.checkLimit(request.userId || 'anonymous', userTier);
            if (!rateLimitCheck.allowed) {
                return this.createErrorResponse('RATE_LIMIT_EXCEEDED', `Rate limit exceeded. Reset time: ${rateLimitCheck.resetTime}`, requestId, startTime);
            }
            ...
            // Update rate limiter
            await this.rateLimiter.recordUsage(request.userId || 'anonymous', claudeResponse.metadata.tokensUsed);
            return {
                success: true,
                data: {
                    ...claudeResponse,
            ...
            return this.createErrorResponse(aiError.code, serviceError.userMessage || aiError.message, requestId, startTime);
        }
    }
    /**
     ...
     */
    async callClaudeAPI(prompt, domain) {
        const startTime = Date.now();
     ...
     */
    async getDomainAnalytics(domain, timeframe = '24h') {
    ...
    createErrorResponse(code, message, requestId, startTime) {
        return {
            success: false,
            error: {
                code,
...
export const claudeOrchestrator = new ClaudeOrchestrator(process.env.ANTHROPIC_API_KEY);
