...
export class CorporateAIService {
  private domain: DomainType = 'corporate';

  /**
   * Generate corporate content
   */
  async generateCorporateContent(
    request: CorporateContentRequest,
    userId: string,
    userRole: UserRole,
    tier: RateLimitTier
    ...
    return await aiService.generateContent(
      prompt,
      this.domain,
      'corporate_content' as ContentType,
      userId,
      userRole,
      tier
    );
...
Format: Professional executive summary with clear sections and compelling narrative.`;
    ...
    return await aiService.generateContent(
      prompt,
      this.domain,
      'executive_summary' as ContentType,
      userId,
      userRole,
      tier
    );
   ...
   */
  async generateLeadershipProfile(
    leader: LeadershipContent,
    userId: string,
    userRole: UserRole,
    tier: RateLimitTier
  ): Promise<ServiceResponse<string>> {
...
Format: Professional biography suitable for corporate website or investor materials.`;
    ...
    return await aiService.generateContent(
      prompt,
      this.domain,
      'leadership_content' as ContentType,
      userId,
      userRole,
      tier
    );
...
Format: Professional investor communication with executive summary, detailed sections, and forward-looking statements.`;
    ...
    return await aiService.generateContent(
      prompt,
      this.domain,
      'investor_update' as ContentType,
      userId,
      userRole,
      tier
    );
...
Format: Executive strategic plan with clear structure, timelines, and accountability measures.`;
  ...
  async generatePressRelease(
    announcement: string,
    details: string[],
    quotes: Array<{ speaker: string; role: string; quote: string }>,
...
Format: Standard press release with dateline, headline, body paragraphs, quotes, and boilerplate.`;
   ...
   */
  async generateCompanyAnnouncement(
...
Format: Professional company communication with clear structure and call-to-action.`;
    ...
    return await aiService.generateContent(
      prompt,
      this.domain,
      'company_announcement' as ContentType,
      userId,
      userRole,
      tier
    );
...
Format: Concise, powerful vision statement with supporting narrative.`;
    ...
    return await aiService.generateContent(
      prompt,
      this.domain,
      'vision_statement' as ContentType,
      userId,
      userRole,
      tier
    );
  ...
  private buildCorporateContentPrompt(request: CorporateContentRequest): string {
    const lengthGuide = {
      brief: '200-300 words',
...
6. Includes compelling narrative structure`;
  ...
  private formatMetrics(metrics: CompanyMetrics): string {
    const formatted: string[] = [];
    
    if (metrics.revenue) formatted.push(`• Revenue: $${metrics.revenue.toLocaleString()}`);
    if (metrics.growth) formatted.push(`• Growth: ${metrics.growth}%`);
    if (metrics.employees) formatted.push(`• Employees: ${metrics.employees.toLocaleString()}`);
    if (metrics.clients) formatted.push(`• Clients: ${metrics.clients.toLocaleString()}`);
    if (metrics.projects) formatted.push(`• Projects: ${metrics.projects.toLocaleString()}`);
    if (metrics.satisfaction) formatted.push(`• Client Satisfaction: ${metrics.satisfaction}%`);
